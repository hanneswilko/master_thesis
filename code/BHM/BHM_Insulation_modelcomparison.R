...
#BHM insulation models
m2 <- read_rds("./output/models_rds/fitInsulation_m2.rds") # varying intercept + group level predictor
m3.1 <- read_rds("./output/models_rds/fitInsulation_m3.1.rds") # varying intercept & slope (EPS|country)
m4 <- read_rds("./output/models_rds/fitInsulation_m4.rds") # varying intercept & slope (1+EPS|country) + interaction EPS*Income
...
m4_mcmc_trace <- mcmc_trace(m4) +
  ggtitle("MCMC Trace Plots for Thermal Insulation (m4)") + 
  theme(
    strip.text = element_text(size = 6), # Adjust facet label text size
    plot.title = element_text(size = 14, face = "bold", hjust = 0.5),
    axis.text.x = element_text(angle = 45, hjust = 1)         
...
m4_mcmc_dens_overlay <- mcmc_dens_overlay(m4) +
  ggtitle("MCMC Density Plots for Thermal Insulation (m4)") + 
  theme(
    strip.text = element_text(size = 6), # Adjust facet label text size
    plot.title = element_text(size = 14, face = "bold", hjust = 0.5),
    axis.text.x = element_text(angle = 45, hjust = 1)         
...
m4_mcmc_acf_fixed <- mcmc_acf_bar(m4_array, pars = m4_fixed_pars, lags = 10) +
  ggtitle("Autocorrelation Plots for Thermal Insulation (m4) - Fixed Effects") +
  theme(
    strip.text = element_text(size = 5),            
    axis.text = element_text(size = 5),
    axis.title = element_text(size = 6),
...
m4_mcmc_acf_random <- mcmc_acf_bar(m4_array, pars = m4_random_pars, lags = 10) +
  ggtitle("Autocorrelation Plots for Thermal Insulation (m4) - Random Effects") +
  theme(
    strip.text = element_text(size = 5),            
    axis.text = element_text(size = 5),
    axis.title = element_text(size = 6),
...
m4_ppc <- ppc_stat(Adoption, Adoption_rep, stat = "mean") +
  ggtitle("Posterior Predictive Check for Thermal Insulation (m4)") + 
  labs(x = "Adoption Rate") +
  theme(
    axis.title = element_text(size = 12, face = "bold"),
    plot.title = element_text(size = 14, face = "bold", hjust = 0.5)   
...
variables_of_interest <- c("(Intercept)", "Female", "Higher_edu",
                           "Incomequintile 5", "Dwelling_sizeMore than 200 m²",
...
set.seed(84735)
m2_postclass <- classification_summary(model = m2, data = insulation, cutoff = 0.5)
m3.1_postclass <- classification_summary(model = m3.1, data = insulation, cutoff = 0.5)
m4_postclass <- classification_summary(model = m4, data = insulation, cutoff = 0.5)
...
# Output directory:
output_dir <- "./output/tables_tex/insulation_tables"
...
#--------------------------- Output Plots ---------------------------------------
##m2
ggsave("./output/output_insulation/m2_mcmc_trace.pdf", plot = m2_mcmc_trace, dpi = 300, scale = 1.2)
ggsave("./output/output_insulation/m2_mcmc_acf_fixed.pdf", plot = m2_mcmc_acf_fixed, dpi = 300, scale = 1.2)
ggsave("./output/output_insulation/m2_mcmc_acf_random.pdf", plot = m2_mcmc_acf_random, dpi = 300, scale = 1.2)
ggsave("./output/output_insulation/m2_mcmc_dens_overlay.pdf", plot = m2_mcmc_dens_overlay, dpi = 300, scale = 1.2)
ggsave("./output/output_insulation/m2_ppc.pdf", plot = m2_ppc, dpi = 300, scale = 1.2)

##m3.1
ggsave("./output/output_insulation/m3.1_mcmc_trace.pdf", plot = m3.1_mcmc_trace, dpi = 300, scale = 1.2)
ggsave("./output/output_insulation/m3.1_mcmc_acf_fixed.pdf", plot = m3.1_mcmc_acf_fixed, dpi = 300, scale = 1.2)
ggsave("./output/output_insulation/m3.1_mcmc_acf_random.pdf", plot = m3.1_mcmc_acf_random, dpi = 300, scale = 1.2)
ggsave("./output/output_insulation/m3.1_mcmc_dens_overlay.pdf", plot = m3.1_mcmc_dens_overlay, dpi = 300, scale = 1.2)
ggsave("./output/output_insulation/m3.1_ppc.pdf", plot = m3.1_ppc, dpi = 300, scale = 1.2)

##m4
ggsave("./output/output_insulation/m4_mcmc_trace.pdf", plot = m4_mcmc_trace, dpi = 300, scale = 1.2)
ggsave("./output/output_insulation/m4_mcmc_acf_fixed.pdf", plot = m4_mcmc_acf_fixed, dpi = 300, scale = 1.2)
ggsave("./output/output_insulation/m4_mcmc_acf_random.pdf", plot = m4_mcmc_acf_random, dpi = 300, scale = 1.2)
ggsave("./output/output_insulation/m4_mcmc_dens_overlay.pdf", plot = m4_mcmc_dens_overlay, dpi = 300, scale = 1.2)
ggsave("./output/output_insulation/m4_ppc.pdf", plot = m4_ppc, dpi = 300, scale = 1.2)
