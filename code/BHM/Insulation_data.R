#-------------------------------------------------------------------------------
#-------------------- Data - Thermal Insulation - Model setup ------------------
#-------------------------------------------------------------------------------
# 1. Loading Packages
# 2. Loading Data
# 3. Cleaning Data
...
#C49_1: In support of providing subsidies to households for purchasing energy-efficient Insulation or investing in renewable energy equipment
#C49_2: In support of Taxing energy use or the purchase of Insulation and equipment that use a lot of energy
#C49_3: In support of introducing energy efficiency standards for Insulation and buildings that manufacturers have to comply with
...
epic <- epic %>%
  ...
  select(ID, weight, weight_2, Country_code, Country_name, Age_cat, Income, Gender, S5, REGION_UK, REGION_SE3,
         ...
         C37_5, C37_6, C37_8, C49_1, C49_2, C49_3, C50, C44_4, C45_4, C46_4)

#create flag for HHs where respective EET adoption wasn't possible
epic_Insulation <- epic %>%
  mutate(
    # thermal insulation
...
epic_Insulation <- epic_Insulation %>%
  mutate(
    Gov_support = case_when(
      C45_4 == 1 ~ 1, #support
      TRUE ~ 0 #else no support
    ),
    Adoption = ifelse(C44_4 == 1, 1, 0)
  )
...
#Income quintiles
#S9_X: Education
summary(as.factor(epic_Insulation$S9_IL))
summary(as.factor(epic_Insulation$S9_NL))
...
##Strongly disagree = 1, Disagree = 2, Neither agree or disagree = 3, Agree = 4, Strongly agree = 5, Prefer not to say = 999999
#In support of providing subsidies to households for purchasing energy-efficient Insulation or investing in renewable energy equipment
##Strongly against = 1, against = 2, indifferent = 3, support = 4, Strongly support = 5
#In support of Taxing energy use or the purchase of Insulation and equipment that use a lot of energy
##Strongly against = 1, against = 2, indifferent = 3, support = 4, Strongly support = 5
#In support of introducing energy efficiency standards for Insulation and buildings that manufacturers have to comply with
##Strongly against = 1, against = 2, indifferent = 3, support = 4, Strongly support = 5
...
#---------------------------- Selected Data Set  -------------------------------
epic_Insulation_selected <- epic_Insulation %>%
...
#-------------------------- 4. Merging Data ------------------------------------
#-------------------------------------------------------------------------------
Insulation <- epic_Insulation_selected %>%
  left_join(EPS_sub_avg, by = c("Country_name" = "REF_AREA"))

#---------------------------- 4.1 Check Data -----------------------------------
glimpse(Insulation)
unique(Insulation$Adoption)
unique(Insulation$Income)
unique(Insulation$Dwelling_size)

summary(as.factor(Insulation$Dwelling_size)) #coded such that don't know is separate category (since relatively large number)

summary(as.factor(Insulation$Env_concern)) #no NAs
summary(as.factor(Insulation$Env_policy_public)) #no NAs
summary(as.factor(Insulation$Env_policy_costs)) #no NAs
summary(as.factor(Insulation$Env_policy_subsidy)) #no NAs
summary(as.factor(Insulation$Env_policy_tax)) #no NAs
summary(as.factor(Insulation$Env_policy_standards)) #no NAs
summary(as.factor(Insulation$Env_policy_liH)) #no NAs
#recoded NAs with environmental policy preferences since relatively small number only ~1%

#---------------------------- 4.2 Factoring Data -------------------------------
#Dwelling size
Insulation$Dwelling_size = factor(Insulation$Dwelling_size,
...
Insulation$Dwelling_size <- relevel(Insulation$Dwelling_size, ref = "Less than 25 m²")


# Age category
Insulation$Age_cat <- factor(Insulation$Age_cat,
                             levels = 1:5,
                             labels = c("18-24", "25-34", "35-44", "45-54", "55+"))

Insulation$Age_cat <- relevel(Insulation$Age_cat, ref = "18-24")


# Income quintile
Insulation$Income <- factor(Insulation$Income,
                            levels = 1:5,
                            labels = c("quintile 1", "quintile 2", "quintile 3", "quintile 4", "quintile 5"))

Insulation$Income <- relevel(Insulation$Income, ref = "quintile 1")
...
table(Insulation$Country_name)

#distribution of key variables per country
Insulation %>% group_by(Country_name) %>% summarise(across(c(Income, Age_cat, Dwelling_size), ~n_distinct(.)))

#checking NAs
missing_data <- colSums(is.na(Insulation))
print(missing_data)

# Cross-tabulation of Dwelling_size by Country_name
dwelling_distribution <- table(Insulation$Country_name, Insulation$Dwelling_size)
...
#--------------------------- 5. Saving Data ------------------------------------
#-------------------------------------------------------------------------------
write.csv(Insulation, "./processed_data/insulation.csv", row.names = FALSE)
