...
epic <- epic %>%
  ...
  select(ID, weight, weight_2, Country_code, Country_name, Age_cat, Income, Gender, S5, REGION_UK, REGION_SE3,
         ...
         C37_5, C37_6, C37_8, C49_1, C49_2, C49_3, C50, C44_3, C45_3, C46_3)
...
#Income quintiles
#S9_X: Education
summary(as.factor(epic_Windows$S9_IL))
summary(as.factor(epic_Windows$S9_NL))
...
#---------------------------- Selected Data Set  -------------------------------
epic_Windows_selected <- epic_Windows %>%
...
#-------------------------- 4. Merging Data ------------------------------------
#-------------------------------------------------------------------------------
Windows <- epic_Windows_selected %>%
  left_join(EPS_sub_avg, by = c("Country_name" = "REF_AREA"))

#---------------------------- 4.1 Check Data -----------------------------------
glimpse(Windows)
unique(Windows$Adoption)
unique(Windows$Income)
unique(Windows$Dwelling_size)

summary(as.factor(Windows$Dwelling_size)) #coded such that don't know is separate category (since relatively large number)

summary(as.factor(Windows$Env_concern)) #no NAs
summary(as.factor(Windows$Env_policy_public)) #no NAs
summary(as.factor(Windows$Env_policy_costs)) #no NAs
summary(as.factor(Windows$Env_policy_subsidy)) #no NAs
summary(as.factor(Windows$Env_policy_tax)) #no NAs
summary(as.factor(Windows$Env_policy_standards)) #no NAs
summary(as.factor(Windows$Env_policy_liH)) #no NAs
#recoded NAs with environmental policy preferences since relatively small number only ~1%

#---------------------------- 4.2 Factoring Data -------------------------------
#Dwelling size
Windows$Dwelling_size = factor(Windows$Dwelling_size,
...
Windows$Dwelling_size <- relevel(Windows$Dwelling_size, ref = "Less than 25 m²")


# Age category
Windows$Age_cat <- factor(Windows$Age_cat,
                             levels = 1:5,
                             labels = c("18-24", "25-34", "35-44", "45-54", "55+"))

Windows$Age_cat <- relevel(Windows$Age_cat, ref = "18-24")


# Income quintile
Windows$Income <- factor(Windows$Income,
                            levels = 1:5,
                            labels = c("quintile 1", "quintile 2", "quintile 3", "quintile 4", "quintile 5"))

Windows$Income <- relevel(Windows$Income, ref = "quintile 1")
...
table(Windows$Country_name)

#distribution of key variables per country
Windows %>% group_by(Country_name) %>% summarise(across(c(Income, Age_cat, Dwelling_size), ~n_distinct(.)))

#checking NAs
missing_data <- colSums(is.na(Windows))
print(missing_data)

# Cross-tabulation of Dwelling_size by Country_name
dwelling_distribution <- table(Windows$Country_name, Windows$Dwelling_size)
...
#--------------------------- 5. Saving Data ------------------------------------
#-------------------------------------------------------------------------------
write.csv(Windows, "./processed_data/windows.csv", row.names = FALSE)
