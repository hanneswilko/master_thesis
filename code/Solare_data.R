...
#C44_6: Solare
#C45_6: Government support for Solare
#C46_6: Why not Solare
...
#C49_1: In support of providing subsidies to households for purchasing energy-efficient Solare or investing in renewable energy equipment
#C49_2: In support of Taxing energy use or the purchase of Solare and equipment that use a lot of energy
#C49_3: In support of introducing energy efficiency standards for Solare and buildings that manufacturers have to comply with
...
epic <- epic %>%
  ...
  select(ID, weight, weight_2, Country_code, Country_name, Age_cat, Income, Gender, S5, REGION_UK, REGION_SE3,
         ...
         C37_5, C37_6, C37_8, C49_1, C49_2, C49_3, C50, C44_6, C45_6, C46_6)

#create flag for HHs where respective EET adoption wasn't possible
epic_Solare <- epic %>%
  mutate(
    # Highly energy-efficient Solare
...
epic_Solare <- epic_Solare %>%
  mutate(
    Gov_support = case_when(
      C45_6 == 1 ~ 1, #support
      TRUE ~ 0 #else no support
    ),
    Adoption = ifelse(C44_6 == 1, 1, 0)
  )
...
#Income quintiles
#S9_X: Education
summary(as.factor(epic_Solare$S9_IL))
summary(as.factor(epic_Solare$S9_NL))
...
##Strongly disagree = 1, Disagree = 2, Neither agree or disagree = 3, Agree = 4, Strongly agree = 5, Prefer not to say = 999999
#In support of providing subsidies to households for purchasing energy-efficient Solare or investing in renewable energy equipment
##Strongly against = 1, against = 2, indifferent = 3, support = 4, Strongly support = 5
#In support of Taxing energy use or the purchase of Solare and equipment that use a lot of energy
##Strongly against = 1, against = 2, indifferent = 3, support = 4, Strongly support = 5
#In support of introducing energy efficiency standards for Solare and buildings that manufacturers have to comply with
##Strongly against = 1, against = 2, indifferent = 3, support = 4, Strongly support = 5
...
#---------------------------- Selected Data Set  -------------------------------
epic_Solare_selected <- epic_Solare %>%
...
#-------------------------- 4. Merging Data ------------------------------------
#-------------------------------------------------------------------------------
Solare <- epic_Solare_selected %>%
  left_join(EPS_sub_avg, by = c("Country_name" = "REF_AREA"))

#---------------------------- 4.1 Check Data -----------------------------------
glimpse(Solare)
unique(Solare$Adoption)
unique(Solare$Income)
unique(Solare$Dwelling_size)

summary(as.factor(Solare$Dwelling_size)) #coded such that don't know is separate category (since relatively large number)

summary(as.factor(Solare$Env_concern)) #no NAs
summary(as.factor(Solare$Env_policy_public)) #no NAs
summary(as.factor(Solare$Env_policy_costs)) #no NAs
summary(as.factor(Solare$Env_policy_subsidy)) #no NAs
summary(as.factor(Solare$Env_policy_tax)) #no NAs
summary(as.factor(Solare$Env_policy_standards)) #no NAs
summary(as.factor(Solare$Env_policy_liH)) #no NAs
#recoded NAs with environmental policy preferences since relatively small number only ~1%

#---------------------------- 4.2 Factoring Data -------------------------------
#Dwelling size
Solare$Dwelling_size = factor(Solare$Dwelling_size,
...
Solare$Dwelling_size <- relevel(Solare$Dwelling_size, ref = "Less than 25 m²")


# Age category
Solare$Age_cat <- factor(Solare$Age_cat,
                            levels = 1:5,
                            labels = c("18-24", "25-34", "35-44", "45-54", "55+"))

Solare$Age_cat <- relevel(Solare$Age_cat, ref = "18-24")


# Income quintile
Solare$Income <- factor(Solare$Income,
                           levels = 1:5,
                           labels = c("quintile 1", "quintile 2", "quintile 3", "quintile 4", "quintile 5"))

Solare$Income <- relevel(Solare$Income, ref = "quintile 1")
...
table(Solare$Country_name)

#distribution of key variables per country
Solare %>% group_by(Country_name) %>% summarise(across(c(Income, Age_cat, Dwelling_size), ~n_distinct(.)))

#checking NAs
missing_data <- colSums(is.na(Solare))
print(missing_data)

# Cross-tabulation of Dwelling_size by Country_name
dwelling_distribution <- table(Solare$Country_name, Solare$Dwelling_size)
...
#--------------------------- 5. Saving Data ------------------------------------
#-------------------------------------------------------------------------------
write.csv(Solare, "./processed_data/solare.csv", row.names = FALSE)
